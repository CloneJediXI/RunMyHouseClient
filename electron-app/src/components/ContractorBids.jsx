...
class ContractorBids extends React.Component {
  constructor(props) {
    super(props);
  ...
  getJobs(){
    fetch('http://'+getServer()+':80/runmyhouseserver/job.php?status=Bidding', {method: 'GET', 
  ...
  submitBid(amount, ticket){
    fetch('http://'+getServer()+':80/runmyhouseserver/job.php?ticketId='+ticket+'&bid='+amount+'&cost='+this.state.jobCost+'&contractorId='+this.state.id, {method: 'GET', 
    mode: 'cors', crossDomain:true,})
        .then(response => response.json())
